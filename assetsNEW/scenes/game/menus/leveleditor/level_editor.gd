...
@onready var editorTheme = preload("res://assetsNEW/scenes/game/menus/leveleditor/editorTheme.tres")
@onready var editorLevel = preload("res://assetsNEW/scenes/game/menus/leveleditor/editor_level.gd")
...
func bm_editor_on_load():
	...
	if bconf.open_recent_button:
		if openRecentButton == null:
			openRecentButton = Button.new()
			openRecentButton.text = "Open recently edited level"
			openRecentButton.pressed.connect(_on_open_recent_pressed)
			openRecentButton.position = BingusMod.RECENT_BUTTON_POS
			openRecentButton.add_theme_font_size_override("font_size", 12)
			openRecentButton.add_theme_constant_override("outline_size", 8)
			openRecentButton.set_name(&"openRecent")
			$pickOption.add_child(openRecentButton)
			BingusMod.message("added open recent button woohoo !")

		if openRecentOptionButton == null:
			var recents = BingusMod.load_recent_levels()
			openRecentOptionButton = OptionButton.new()
			openRecentOptionButton.position = BingusMod.RECENT_OPTION_BUTTON_POS
			openRecentOptionButton.add_theme_font_size_override("font_size", 8)
			openRecentOptionButton.add_theme_constant_override("outline_size", 6)
			openRecentOptionButton.set_name(&"openRecentOptions")
			var popup = openRecentOptionButton.get_popup()
			...
			openRecentOptionButton.add_item("--- Choose a level ---")
			openRecentOptionButton.set_item_disabled(0, true)
			openRecentOptionButton.select(0)
			for i in int(bconf.num_recent_levels):
				if recents.size() > i:
			...
			$pickOption.add_child(openRecentOptionButton)
			BingusMod.message("added open recent button ay a !")
		
	if hideSidebarArrow == null && bconf.hide_sidebar_arrow:
		hideSidebarArrow = Button.new()
		hideSidebarArrow.theme = editorTheme
		hideSidebarArrow.position = BingusMod.HIDE_SIDEBAR_POS
		hideSidebarArrow.text = " < "
		hideSidebarArrow.pressed.connect(_on_hide_sidebar_pressed)
		hideSidebarArrow.set_name(&"hideSidebar")
		...
		hideSidebarArrow.add_theme_stylebox_override(&"normal", styleboxNormal)
		hideSidebarArrow.add_theme_stylebox_override(&"normal_mirrored", styleboxNormal)
		...
		hideSidebarArrow.add_theme_stylebox_override(&"hover", styleboxHover)
		hideSidebarArrow.add_theme_stylebox_override(&"hover_mirrored", styleboxHover)
		...
		hideSidebarArrow.add_theme_stylebox_override(&"pressed", styleboxClick)
		hideSidebarArrow.add_theme_stylebox_override(&"pressed_mirrored", styleboxClick)
...
func add_tooltips(base, dict) -> void:
	for key in dict:
		var val = dict[key]
		var node_path = base + key
		if val is Dictionary:
			add_tooltips(node_path + "/", val)
			continue
...
func _on_hide_sidebar_pressed() -> void:
	if sidebarIsOffset:
		...
		$HUD.offset.x = -BingusMod.SIDEBAR_WIDTH
		hideSidebarArrow.text = " > "
	get_parent().get_parent().get_child(1).offset.x = $HUD.offset.x
...
func _process(delta: float) -> void :
	...
	if Input.is_action_just_pressed("middleMouse") && !BingusMod.pan_key_down(bconf):
...
func _input(event):
	...
	if event is InputEventMouseMotion:
		# PATCH: allow using ctrl to pan
		if Input.is_action_pressed("middleMouse") && BingusMod.pan_key_down(bconf):
		...
		if selectedThing != null && selectMode == true:
			if Input.is_action_pressed("alt"):
				if Input.is_action_just_pressed("Click") && get_global_mouse_position().distance_to(selectedThing.global_position) < getObjRect(selectedThing).x / 2:
					...
					dupe.global_position = get_global_mouse_position()
					add_child(dupe, true)
					# PATCH: prevent crash when duplicating editor deco
					if selectedThing.has("variables"):
						dupe.variables = selectedThing.variables.duplicate()
					selected(dupe)
					selectedThing = dupe
					updateHistory()
		...
		if "nodes" in saveData:
			for i in saveData.nodes:
				...
				for p in objectScenes:
					...
					pp += 1
				# PATCH: prevent crash when object has no properties
				if i[5] != "sand" and i[3] != null:
					for j in i[3].keys():
						if i[3].get(j) != null:
							d.set(j, i[3].get(j))
				else:
...
# PATCH: detect when file is opened
